...
namespace BibliotecaMaf.Controles.BloqueoAplicacion
{
	public partial class Bloqueo : UserControl
	{
		...
		private void BloqueoSowart_Load(object sender, System.EventArgs e)
		{
			...
			if (TestKey(Application.StartupPath + "\\CodigoSowart.regkey") == true)
			{
				if (DesBloqueado != null)
				{
					DesBloqueado(this, new EventArgs());
			...
			else
			{
				...
				{
					if (TestKey(mUnidade.RootDirectory + "CodigoSowart.regkey") == true)
					{
						if (DesBloqueado != null)
						{
							DesBloqueado(this, new EventArgs());
			...
			if (System.IO.File.Exists(Application.StartupPath + "\\CodigoSowart.regkey"))
			{
				...
				mInformacion = "MicroID: " + HardwareData.IdProcesador;
			...
			{
				mInformacion = "MicroID: " + HardwareData.IdProcesador + "\r\n";
				mInformacion = mInformacion + "Procesador: " + HardwareData.NombreProcesador + " - " + HardwareData.BitsProcesador + "\r\n";
		...
		private bool TestKey(string pathKey)
		{
			if (System.IO.File.Exists(pathKey))
			{
				string mClaveLocal = System.IO.File.ReadAllText(pathKey);
				mInformacion = "MicroID: " + HardwareData.IdProcesador;
				if (mClaveLocal == Rutinas_Generales.Encrypt_Decrypt(mInformacion, Rutinas_Generales.Clave2, Rutinas_Generales.Iniciador2, true))
				{
					return true;
				}
			}
		...
		private void timerBuscandoLlave_Tick(object sender, EventArgs e)
		{
			...
			{
				string Dir = d.RootDirectory.Name + "4b2740203b5d306e5c2355366b.dat";

				if (System.IO.File.Exists(Dir))
				{
					if (System.IO.File.ReadAllText(Dir) == "2a6a524e7c34706d6f617a6a2e63693940204267")
					{
